...
func HttpPosthb(Url string, data io.Reader, headers *map[string]string, ua string, proxyAddr string, proxyUser string, proxyPass string) string {
	resp, err := WxHttpRequesthb(Url, "POST", headers, data, ua, proxyAddr, proxyUser, proxyPass)
	if err != nil {
		return err.Error()
	}
	if resp == nil {
